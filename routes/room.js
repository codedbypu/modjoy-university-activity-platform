...
router.post('/room/:id/check-in', async (req, res) => {
    ...
    const roomId = req.params.id;
    const { code } = req.body; // รับรหัสจากหน้าบ้านมา
    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        const userId = decoded.id;
        // ดึงข้อมูลห้องและก็พวกรหัสที่ถูกต้องกับสมาชิกในห้อง
        const checkSql = `
            SELECT 
                R.ROOM_CHECKIN_CODE,
        ...
        // ส่วนที่เหลือเป็นการเช็คเบื้องต้นครับ เช่น เช็คว่าเป็นสมาชิกไหม รหัสถูกไหม หรือว่าหมดอายุไหม
        ...
        // ถ้ารหัสไม่ผิดก็บันทึกเวลาเช็คชื่อ
        const updateSql = `
            UPDATE ROOMMEMBERS 
...
router.get('/my-activities', async (req, res) => {
    ...
    try {
        ...
        // หลังจากเช็คว่าเป้นหน้าไหนแล้วต่อไป เป็นส่วนการค้นหาและกรอง
        // พิมพ์ค้นหา
        if (search) {
            const isNumeric = !isNaN(search) && search.trim() !== '';
            if (isNumeric) {
        ...
        const sql = `
            SELECT 
                ...
                CASE 
                    WHEN NOW() < TIMESTAMP(r.ROOM_EVENT_DATE, r.ROOM_EVENT_START_TIME) THEN 0
                    WHEN NOW() > TIMESTAMP(r.ROOM_EVENT_DATE, r.ROOM_EVENT_END_TIME) THEN 2
                ELSE 1
                END AS ROOM_STATUS,
...
router.get('/my-joined-active-rooms', async (req, res) => {
    ...
    try {
        ...
        // ส่วนที่เหลือเป็นการเช็คกรณี error
        res.json({ success: true, rooms });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: 'Database error' });
...
router.get('/room/:id', (req, res) => {
    ...
    db.query(sql, [roomId], (err, results) => {
        // ส่วนที่เหลือเป็นการเช็คถ้าไม่พบห้อง หรือ ถ้า error
        if (err) {
            console.error(err);
